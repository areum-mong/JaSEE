# =========================================================
# app_desktop.py ??Fit Me Up ?°ìŠ¤?¬íƒ‘ ?µí•© ë²„ì „
#
# ë³€ê²??¬í•­ (app_desktop_team.py ê¸°ì?):
#   1. integrate.py ?˜ì¡´ ?œê±°
#   2. analyze_image() ??jasee_core ê¸°ë°˜?¼ë¡œ êµì²´
#   3. RealTimePostureProcessor._analyze_frame_background ??jasee_core ê¸°ë°˜
#   4. ?˜ë¨¸ì§€ UI (ë¦¬í¬?? ?ìˆ˜ì¦? ì±Œë¦°ì§€ ?? ê·¸ë?ë¡?? ì?
#
# ?¤í–‰: streamlit run app_desktop.py
# =========================================================
...
from jasee_core import (
    ...
    predict_posture, calc_cva, calc_tia,
    AttentionMLP
)
...
# =========================================================
# 2. analyze_image() ??integrate.py ?€ì²??µì‹¬ ?¨ìˆ˜
#    ê¸°ì¡´ app_desktop_team.py??analyze_image()?€ ?™ì¼??
#    ë°˜í™˜ ?•ì‹ ? ì? ???˜ë¨¸ì§€ UI ì½”ë“œ ë¬´ìˆ˜???¬ìš©
# =========================================================
def analyze_image(pil_image: Image.Image) -> dict:
    """
    jasee_core ê¸°ë°˜ ë¶„ì„ ??integrate.py run_pipeline() ?€ì²?
    ë°˜í™˜ ?•ì‹?€ app_desktop_team.py ê¸°ì¡´ ?•ì‹ê³??¸í™˜ ? ì?:
    {
      ok, gate_pass, gate_bad_items,
      posture: {key: (val_str, is_good, raw)},
      env:     {key: (val_str, is_good, raw)},
    ...
    # ?€?€ ê²°ê³¼ ë³€??(ê¸°ì¡´ ?•ì‹: {key: (val_str, is_good, raw)}) ?€?€
    ...
    if kp:
        ...
        for a, b in [("ê·€","?´ê¹¨"),("?´ê¹¨","ê³¨ë°˜"),("ê³¨ë°˜","ë¬´ë¦"),
                     ("ë¬´ë¦","ë°œëª©"),("?´ê¹¨","?”ê¿ˆì¹?),("?”ê¿ˆì¹?,"?ëª©")]:
    ...
    for label, bbox in bboxes.items():
        ...
        cv2.rectangle(overlay_bgr, (x1,y1-22), (x2,y1), c, -1)
        cv2.putText(overlay_bgr, f"{label} {conf:.0%}",
                    (x1+3,y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 2)
...
# =========================================================
# 3. RealTimePostureProcessor ??jasee_core ê¸°ë°˜?¼ë¡œ êµì²´
#    WebRTC êµ¬ì¡° (recv/snapshot/start/reset) ê·¸ë?ë¡?? ì?
# =========================================================
class RealTimePostureProcessor(VideoProcessorBase):
    """
    ?Œë¡œ??
    1. ?ì„¸ ì¸¡ì • (20ì´? ??ë§??„ë ˆ??17ê´€???¤ì‹œê°??œì¶œ (GOODì´ˆë¡/BADë¹¨ê°•)
    2-A. GOOD 5ì´?? ì? ???˜ê²½ ì¸¡ì • ?¨ê³„ ?ë™ ?„í™˜
    2-B. 20ì´?ê²½ê³¼ BAD ???´ë?ì§€ ?€??+ ?ˆë‚´ ë¬¸êµ¬ + ?¬ì¸¡??? ë„
    3. ?˜ê²½ ì¸¡ì • ??4ê°??´ë˜???™ì‹œ ê°ì? ?„ë£Œ ??result ?€??
    """
    ...
    def __init__(self):
        ...
        self.last_feedback          = "?ì„¸ ì¸¡ì • ?œì‘ ë²„íŠ¼???ŒëŸ¬ì£¼ì„¸??"
        ...
        self.env_detected           = {}
        self.env_bboxes             = {}
        self.env_transition_started = None   # ?„í™˜ ?”ë©´ ?œì‘ ?œê°„

        self.POSTURE_TOTAL          = 20
        self.GOOD_HOLD              = 5
        self.ENV_TRANSITION_SEC     = 3      # ?„í™˜ ?”ë©´ ?œì‹œ ?œê°„
    ...
    def recv(self, frame):
        ...
        if phase == "posture":
            ...
            if started_at:
                ...
                cv2.putText(out, f"?ì„¸: {status}", (15,33),
                            cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255,255,255), 2)
                cv2.putText(out, f"?¨ì?: {remain:.0f}s", (w-130,33),
                ...
                if status == "GOOD" and hold > 0:
                    ...
                    cv2.putText(out, f"GOOD ? ì?: {hold:.1f}s / {self.GOOD_HOLD}s",
    ...
    def _draw_pose_frame(self, img):
        ...
        with self.lock:
            status = self.latest_kp_status

        color = self.GOOD_COL if status=="GOOD" else self.BAD_COL if status=="BAD" else (180,180,180)
        ...
        for r in results:
            ...
            for kp_data in r.keypoints.data:
                ...
                for idx in range(17):
                    ...
                    cv2.circle(out, (int(kp[idx][0]), int(kp[idx][1])), 7, (255,255,255), 1)
                break
            break
        return out

    # ?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€
    # ?˜ê²½ ì¸¡ì • ?„í™˜ ?”ë©´ (?¨ìƒ‰ ë°°ê²½ + ë¬¸êµ¬)
    # ?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€
    def _draw_transition_frame(self, img, now):
        h, w = img.shape[:2]
        out  = np.zeros((h, w, 3), dtype=np.uint8)
        out[:] = (29, 158, 117)   # ë¯¼íŠ¸ ?¨ìƒ‰ ë°°ê²½
        ...
        lines_text = [
            ...
            "",
            "?´ì œ ?‘ì—…?˜ê²½??ì¸¡ì •?©ë‹ˆ??",
            "ì¹´ë©”?¼ì— ì±…ìƒ / ?˜ì / ëª¨ë‹ˆ?°ê?",
            "ëª¨ë‘ ë³´ì´?„ë¡ ?„ì¹˜ë¥?ì¡°ì •?´ì£¼?¸ìš”.",
        ]
        y_start = h // 2 - 80
    ...
    def _draw_env_frame(self, img, now):
        ...
        for r in env_results:
            for box in r.boxes:
                ...
                cv2.putText(out,f"{label} {conf:.0%}",(x1+3,y1-5),
        ...
        try:
            ...
            with self.lock:
                ...
                if result.get("ok") and result.get("gate_pass", False):
                    ...
                    self.last_feedback     = f"GOOD ? ì?: {self.good_hold_seconds:.1f}s / {self.GOOD_HOLD}s"
                    self.latest_result     = result
                    ...
                    self.last_feedback = " / ".join(bad_items[:2]) if bad_items else "BAD ?ì„¸?…ë‹ˆ??"
        finally:
            with self.lock:
                self.analysis_running = False
    ...
    def start(self):
        with self.lock:
            ...
            self.last_status            = "WAIT"
            self.latest_kp_status       = "WAIT"
            self.last_feedback          = f"{self.POSTURE_TOTAL}ì´??ˆì— GOOD ?ì„¸ë¥?{self.GOOD_HOLD}ì´?? ì??´ì£¼?¸ìš”."
            self.latest_result          = None
            self.latest_keypoints       = None
            self.good_ready             = False
            self.finished_bad           = False
...
def render_auth_page():
    """
    ë¡œê·¸???Œì›ê°€???”ë©´
    - ë¡œê³ : logo.png ??ë°°ê²½ ?ë™ ?¬ëª… ì²˜ë¦¬
    - ë¡œê·¸???Œì›ê°€??? íƒ ?? ë¡œê³  ì¢Œì¸¡? ì— ë§ì¶¤
    - ?…ë ¥ì°?ë²„íŠ¼: ë¡œê³ ë³´ë‹¤ ?´ì§ ?¬ê²Œ ì¤‘ì•™ ?•ë ¬
    - HTML img ?œê·¸ë¥??¬ìš©?˜ì? ?Šì•„ Python ë¬¸ë²• ?¤ë¥˜ ë°©ì?
    """
...
/* ë¡œê·¸???Œì›ê°€??? íƒ ?ì—­: ë¡œê³  ì¢Œì¸¡ ?œì‘? ê³¼ ë§ì¶¤ */
...
div[data-testid="stButton"] button {
    ...
    with center:
        # ë¡œê³ ??380px, ?…ë ¥ì°½ì? CSSë¡?430px ???…ë ¥ì°½ì´ ë¡œê³ ë³´ë‹¤ ?´ì§ ??
...
<div style="width:380px;text-align:left;font-size:34px;font-weight:900;color:#172033;margin-top:20px;">
    Fit Me Up
        ...
        if auth_tab == "ë¡œê·¸??:
            st.markdown(
                '<div class="auth-section-title">ë¡œê·¸??/div>',
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                '<div class="auth-section-title">?Œì›ê°€??/div>',
                unsafe_allow_html=True,
            )
...
def render_pretty_7_metric_dashboard(result):
    ...
    <div class="feedback-grid">
        ...
        <div class="ai-feedback-box blue">
            <div class="ai-feedback-subtitle blue">ì¶”ì²œ ?´ë™ ë°??¤íŠ¸?ˆì¹­</div>
...
<div class="fit-card">
    ...
    <div style="font-size:13px;line-height:1.8;color:#667085;">
        ...
        with c1:
            start_btn = st.button("???ì„¸ ì¸¡ì • ?œì‘", use_container_width=True,
        ...
        if processor is None:
            st.info("ì¹´ë©”??ê¶Œí•œ???ˆìš©?˜ë©´ ?¤ì‹œê°?ë¶„ì„???œì‘?????ˆìŠµ?ˆë‹¤.")
            return

        if start_btn:
            processor.start()
            st.session_state.rt_phase        = "posture"
...
<div class="fit-card" style="border-left:6px solid #D94A4A;background:linear-gradient(135deg,#FFF1F1,#fff);">
    ...
    <div style="font-size:14px;line-height:1.9;color:#172033;">
        ?„ì‰½ê²Œë„ 20ì´??™ì•ˆ GOOD ?ì„¸ê°€ ? ì??˜ì? ?Šì•˜?´ìš”.<br>
        ?‘ì—…?˜ê²½ ì¸¡ì •?€ ì§„í–‰?˜ì? ?Šì•˜ì§€ë§??ì„¸ ì¸¡ì • ê²°ê³¼???€?¥í•  ???ˆì–´??<br><br>
        <b>?„ë˜ ?´ë?ì§€???Œë? ?”ì‚´??ë°©í–¥?€ë¡??ì„¸ë¥?êµì •</b>?´ì£¼?¸ìš”.<br>
        êµì • ???¬ì¸¡?•í•˜ë©??‘ì—…?˜ê²½ ì¸¡ì •ê¹Œì? ì§„í–‰?????ˆì–´??
    </div>
</div>""", unsafe_allow_html=True)

            result = snapshot.get("latest_result")
            if result and result.get("overlay") is not None:
                st.image(result["overlay"], caption="BAD ?ì„¸ ???Œë? ?”ì‚´??ë°©í–¥?¼ë¡œ êµì •?´ì£¼?¸ìš”",
        ...
        if phase == "done":
            ...
            if result:
                ...
                st.session_state.rt_ready_result = result
                st.session_state.rt_phase        = "stopped"
                st.success("???ì„¸ + ?‘ì—…?˜ê²½ ì¸¡ì • ?„ë£Œ! ?„ë˜ ê¸°ë¡?˜ê¸° ë²„íŠ¼???ŒëŸ¬ì£¼ì„¸??")
        ...
        if result_to_show and result_to_show.get("ok"):
            ...
            with c1: metric_card(result_to_show.get("score", 0), "ì¢…í•© ?ì„¸ ?ìˆ˜", "#2563EB")
            with c2: metric_card(result_to_show.get("risk", "-"), "?„í—˜??, "#F59E0B")
            with c3: metric_card(f"{good}/{total}", "?‘í˜¸ ì§€??, "#10B981")
            with c4: metric_card(f"{rate}%", "?•ìƒ ë²”ìœ„ ë¹„ìœ¨", "#6366F1")

            img_col, summary_col = st.columns([1.05, 0.95])
            with img_col:
                overlay = result_to_show.get("overlay")
                if overlay is not None:
                    caption = "?ì„¸ ?¤ë²„?ˆì´" if result_to_show.get("gate_pass") else \
                              "BAD ?ì„¸ ???Œë? ?”ì‚´??ë°©í–¥?¼ë¡œ êµì •?´ì£¼?¸ìš”"
            ...
            if st.session_state.get("rt_phase") in ("stopped", "saved", "ready"):
                if st.button("?’¾ ê¸°ë¡?˜ê¸°", use_container_width=True, key="rt_save_history_btn"):
                    if not st.session_state.get("rt_saved"):
                        ...
                        st.success("??ì¸¡ì • ?´ë ¥, ì±Œë¦°ì§€ ?¬ì¸?¸ì— ê¸°ë¡?˜ì—ˆ?µë‹ˆ??")
                    else:
...
""",
    ...
    if result:
        ...
        with c1:
            metric_card(result["score"], "ì¢…í•© ?ì„¸ ?ìˆ˜", "#2563EB")
        with c2:
            metric_card(result["risk"], "?„í—˜??, "#F59E0B")
        with c3:
            metric_card(f"{result['good_count']}/{result['total_count']}", "?‘í˜¸ ì§€??, "#10B981")
        with c4:
...
st.sidebar.markdown("---")
st.sidebar.markdown("#### ?œë¹„???íƒœ")
st.sidebar.caption("YOLO Â· MediaPipe ê¸°ë°˜ ?ì„¸ ë¶„ì„")
st.sidebar.caption("ì¸¡ë©´ ?¬ì§„ ê¸°ì?")
st.sidebar.caption("?˜ë£Œ ì§„ë‹¨???„ë‹Œ ?ì„¸ ?„í—˜??ì°¸ê³ ??)

st.sidebar.markdown("---")
st.sidebar.caption(f"ë¡œê·¸??ê³„ì •: {st.session_state.username}")

if st.sidebar.button("ë¡œê·¸?„ì›ƒ", use_container_width=True):
    st.session_state.logged_in = False
...
<div class="fit-card" style="border-left:6px solid #1D9E75;">
    ...
    <div style="font-size:13px;color:#667085;margin-bottom:10px;">
        ì±…ìƒÂ·?˜ìÂ·ëª¨ë‹ˆ?°ë? ì¹´ë©”?¼ì— ë§ê²Œ ì¡°ì •?´ì£¼?¸ìš”.
    </div>
...
def make_musculoskeletal_report_pdf(result):
    ...
    story.append(Paragraph("Fit Me Up ê·¼ê³¨ê²©ê³„ ë¦¬í¬??, styles["KTitle"]))
    story.append(
        Paragraph(
