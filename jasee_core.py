...
def predict_posture(mlp, cva: float, tia: float, device: str) -> str:
    """AttentionMLP에서 규칙 기반으로 전환. 사유: validate_jasee.py 검증 결과
    규칙이 모델보다 정확함(사람 라벨 238장 기준 Accuracy 0.811 vs 0.578)"""
...
def cv2_put_korean(img: np.ndarray, text: str, pos: tuple,
    ...
    """OpenCV BGR 이미지에 한글 텍스트를 PIL 경유로 그립니다."""
    try:
        ...
        if _fp:
            try:
                font = _ImageFont.truetype(_fp, font_size)
            except Exception:
                font = None
        if font is None:
...
def draw_image_overlay(
    ...
    """
    이미지 분석 결과(7개 지표 전체)를 BGR 프레임에 오버레이합니다.

    Args:
        frame    : BGR numpy 배열
        kp       : {관절명: (x,y)} — run_posture의 keypoints 반환값
        all_kr   : {한글키: (val_str, is_ok, raw)} — posture+env 통합
        bboxes   : {label: [x1,y1,x2,y2]} — 환경 객체 bbox
        detected : {label: conf} — 환경 객체 신뢰도
        metrics  : {영어키: float} — 각도/거리 원시값 (우측 수치 표시용)

    Returns:
        오버레이가 적용된 BGR 배열
    """
    ARROW_COL = COL_ARROW  # (255, 100, 0) — 파랑 화살표
    ...
    if kp:
        ...
        if cva_col == COLOR_BAD and "귀" in kp and "어깨" in kp:
            ...
            out = cv2_put_korean(out, "머리를 뒤로", (ear[0] + 5, ear[1] - 28), 20, ARROW_COL)
        ...
        if tia_col == COLOR_BAD and "어깨" in kp:
            ...
            out = cv2_put_korean(out, "몸통 세우기", (sh[0] + 8, sh[1] - 76), 20, ARROW_COL)
        ...
        if knee_col == COLOR_BAD and "무릎" in kp:
            ...
            if k_raw is not None:
                if float(k_raw) < 85:
                    ...
                    out = cv2_put_korean(out, "의자 낮추기", (knee[0] + 8, knee[1] + 75), 20, ARROW_COL)
                else:
                    target = (knee[0], knee[1] - 70)
                    cv2.arrowedLine(out, knee, target, ARROW_COL, 10, tipLength=0.4)
                    out = cv2_put_korean(out, "의자 올리기", (knee[0] + 8, knee[1] - 80), 20, ARROW_COL)
        ...
        if wrist_col == COLOR_BAD and "손목" in kp:
            ...
            out = cv2_put_korean(out, "손목 중립", (wrist[0] - 90, wrist[1] - 14), 20, ARROW_COL)
    ...
    if chair_bbox and "골반" in kp:
        ...
        if chair_col == COLOR_BAD:
            out = cv2_put_korean(out, "등 밀착 필요", (back_x + 5, back_y1 - 10), 18, ARROW_COL)
