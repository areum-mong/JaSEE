...
#   4. layout="wide", ???˜ì´ì§€ ??ë°°ì¹˜ ?ìš©
#   5. ?˜ë¨¸ì§€ UI ?„ë? ê·¸ë?ë¡?? ì?
#
# ?¤í–‰: streamlit run app_web.py
# =========================================================
...
# =========================================================
# 1. ê¸°ë³¸ ?¤ì • (?? wide + sidebar collapsed)
# =========================================================
st.set_page_config(
...
# =========================================================
# cv2_put_korean ??jasee_core.pyë¡??´ê?
...
def analyze_image(pil_image: Image.Image) -> dict:
    ...
    # 1ì°??ì„¸ ë¶„ì„: ?¬ì¦ˆ ?¤í¬?¸íŠ¸ë¥?ë¨¼ì? ?»ìŠµ?ˆë‹¤.
    # ???¨ê³„?ì„œ???„ì§ ìµœì‹  ?˜ê²½ bboxê°€ ?†ì„ ???ˆìœ¼ë¯€ë¡?
    # ?˜ê²½ ì§€??gaze_angle/desk_diff/chair_gap)???„ë˜?ì„œ ?¤ì‹œ ê³„ì‚°?©ë‹ˆ??
    ...
    # ?¬ì¦ˆ ?¤í¬?¸íŠ¸ ê¸°ë°˜?¼ë¡œ ?˜ê²½ ê°ì²´ë¥??¸ì‹?©ë‹ˆ??
    _kp_raw = posture_out.get("keypoints_raw")  # jasee_core?ì„œ raw numpy ë°°ì—´
    ...
    # ìµœì‹  ?˜ê²½ bboxë¥?session_state???€?¥í•©?ˆë‹¤.
    # ?˜ê²½ ê°ì²´ê°€ ?¼ì‹œ?ìœ¼ë¡??„ë½???„ë ˆ?„ì—?œëŠ” ì§ì „ bboxë¥?? ì???ê³„ì‚° ?Šê???ì¤„ì…?ˆë‹¤.
    ...
    # 2ì°??ì„¸ ë¶„ì„: ë°©ê¸ˆ ?¸ì‹??ìµœì‹  ?˜ê²½ bboxë¡??˜ê²½ ì§€?œê¹Œì§€ ?¤ì‹œ ê³„ì‚°?©ë‹ˆ??
    # ??ê³¼ì •??ê±°ì³ ?¤ë²„?ˆì´?ëŠ” ê°ì²´ê°€ ë³´ì´?”ë° ê²°ê³¼ì°½ì? '?¸ì‹ ë¶ˆê?'ë¡??¨ëŠ” ë¬¸ì œë¥?ë°©ì??©ë‹ˆ??
    ...
    # ?¤ë²„?ˆì´ ê·¸ë¦¬ê¸???jasee_core.draw_image_overlay() ?ì„œ ?¼ê´„ ì²˜ë¦¬
...
def render_auth_page():
    """
    ë¡œê·¸???Œì›ê°€???”ë©´
    - ë¡œê·¸????ëª¨ë°”???”ë©´ê³?ê°™ì? 430px ?„ë ˆ??? ì?
    - ë¡œê³ ê°€ ê³¼í•˜ê²?ì»¤ì?ê±°ë‚˜ ?„ì•„???¬ë°±???ê¸°??ë¬¸ì œ ?´ê²°
    - auth ?¼ë²¨ ?¨ê?
    - ë¡œê·¸???Œì›ê°€???? ?…ë ¥ì°? ë²„íŠ¼??ëª¨ë°”????— ë§ê²Œ ?•ë ¬
    """
...
# ???˜ì´ì§€ ?”ë©´?ì„œ???ë‹¨ ë©”ë‰´ë¥?? ì??˜ê³ , ë³¸ë¬¸ ??³¼ ê·¸ë¦¬?œë§Œ ???”ë©´??ë§ê²Œ ?•ì¥?©ë‹ˆ??
...
<div class="web-app-header">
    <div class="web-app-logo">?ì„¸?ˆë´</div>
    <div class="web-app-sub">AI ?ì„¸ ë¶„ì„ ?œë¹„??/div>
    <div class="web-app-user">ë¡œê·¸??ê³„ì • Â· {st.session_state.username}</div>
</div>
""",
    unsafe_allow_html=True,
)
...
if st.button("ë¡œê·¸?„ì›ƒ", key="web_logout_btn", use_container_width=True):
    st.session_state.logged_in = False
...
/* === WEB PAGE MODE: ê¸°ëŠ¥?€ ê·¸ë?ë¡??ê³  ?”ë©´ ??ë°°ì¹˜ë§??°ìŠ¤?¬í†±?©ìœ¼ë¡??•ì¥ === */
[data-testid="stSidebar"],
section[data-testid="stSidebar"],
[data-testid="collapsedControl"] {
    display: none !important;
...
/* ë©”ë‰´/ë¡œê·¸?„ì›ƒ ?ì—­?€ ????—???ˆë¬´ ?“ì–´ì§€ì§€ ?Šê²Œ ?•ëˆ */
...
/* ?¹ì—?œëŠ” Streamlit ì»¬ëŸ¼???ë˜ì²˜ëŸ¼ ê°€ë¡?ë°°ì¹˜ */
div[data-testid="stHorizontalBlock"] {
    flex-direction: row !important;
    flex-wrap: nowrap !important;
...
/* ë¡œê·¸???”ë©´?€ ?¹ì—?œë„ ì¹´ë“œ?•ìœ¼ë¡?ì¤‘ì•™ ë°°ì¹˜ */
.auth-card {
    max-width: 520px !important;
