...
def analyze_image(pil_image: Image.Image) -> dict:
    ...
    # 1차 자세 분석: 환경 YOLO 필터링에 필요한 사람 키포인트를 먼저 얻습니다.
    # 이 단계에서는 이전 env_bboxes가 없을 수 있으므로 환경 지표가 None일 수 있습니다.
    ...
    # 환경 YOLO 실행: monitor / desk_surface / chair_back / chair_seat bbox를 최신 프레임에서 얻습니다.
    ...
    # 환경 bbox가 감지된 경우에만 세션에 저장합니다.
    # 빈 dict로 덮어쓰면 다음 분석에서 정상 bbox까지 사라질 수 있어 방지합니다.
    if bboxes:
        st.session_state.env_bboxes = bboxes
    else:
        bboxes = st.session_state.get("env_bboxes", {}) or {}

    # 2차 자세/환경 지표 계산: 최신 환경 bbox를 넣어 gaze_angle / desk_diff / chair_gap을 다시 계산합니다.
    try:
    ...
    if kp:
        ...
        for (a, b), lc in LINE_COL.items():
            if a in kp and b in kp:
                cv2.line(overlay_bgr, kp[a], kp[b], (0,0,0), 20)  # 검정 외곽
                cv2.line(overlay_bgr, kp[a], kp[b], lc, 12)        # 컬러 선
        ...
        # 무릎 BAD: 실제 각도 기반 방향 결정
        if knee_col == COLOR_BAD and "무릎" in kp:
            knee   = kp["무릎"]
            k_raw  = (all_kr.get("무릎") or (None,None,None))[2]
            if k_raw is not None:
                if float(k_raw) < 85:
                    # 각도 너무 작음 → 의자 낮추기 ↓
                    target = (knee[0], knee[1] + 70)
                    cv2.arrowedLine(overlay_bgr, knee, target, ARROW_COL, 10, tipLength=0.4)
                    overlay_bgr = cv2_put_korean(overlay_bgr, "의자 낮추기", (knee[0]+8, knee[1]+75), 20, ARROW_COL)
                else:
                    # 각도 너무 큼 → 의자 올리기 ↑
                    target = (knee[0], knee[1] - 70)
                    cv2.arrowedLine(overlay_bgr, knee, target, ARROW_COL, 10, tipLength=0.4)
                    overlay_bgr = cv2_put_korean(overlay_bgr, "의자 올리기", (knee[0]+8, knee[1]-80), 20, ARROW_COL)
    ...
    if chair_bbox and "골반" in kp:
        hip_pt   = kp["골반"]
        back_x   = chair_bbox[2]   # chair_back 우측 끝
        ...
        if chair_col == COLOR_BAD:
            overlay_bgr = cv2_put_korean(overlay_bgr, "등 밀착 필요",
...
/* auth-card CSS는 더 이상 HTML 태그로 묶이지 않으므로 
   원하신다면 .block-container 전체 배경으로 적용하거나 지우셔도 무방합니다. */
.auth-card {
    width: 100%;
    box-sizing: border-box;
    background: #FFFFFF;
...
.element-container {
    ...
    # ❌ [수정됨] 흰색 박스를 만들던 시작 div 태그를 삭제했습니다.
...
<div class="mobile-app-header">
    <div class="mobile-app-logo">자세히봐</div>
...
/* AI 챗봇 입력 패널 최종 보정: 선택박스와 입력창 폭을 맞추고 버튼을 같은 줄에 고정 */
        ...
        try:
            ...
            with self.lock:
                ...
                if isinstance(result, dict) and result.get("ok"):
                    ...
                    # [방법 B] 카메라 오버레이는 draw_posture_overlay() 사용
                    # → analyze_image()의 큰 점/선(overlay) 대신
                    #   jasee_core의 작은 점/선으로 그린 카메라 전용 오버레이 적용
